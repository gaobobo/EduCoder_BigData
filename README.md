...
该脚本用于通过头歌的单片机实践项目。支持的关卡参见本仓库的 [`/scripts/`](./scripts/) 或参见 [微信文章](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=Mzg4Njg3NDQ2Ng==&action=getalbum&album_id=3877388746747985926#wechat_redirect) 。
...
```shell
# Gitee镜像（中国）：
wget -O /tmp/start.sh https://gitee.com/coconut_floss/EduCoder_BigData/raw/dev/start.sh
# 或使用Github：
wget -O /tmp/start.sh https://raw.githubusercontent.com/gaobobo/EduCoder_BigData/main/start.sh
...
不需要克隆本仓库，只需要下载仓库根目录下的[`start.sh`](https://github.com/gaobobo/EduCoder_BigData/blob/main/start.sh)，该脚本会自动下载所需文件，不会拷贝完整仓库。
...
- `<Level>`：关卡代码。该代码是[`/scripts/`](./scripts/)中的文件名，不需要加后缀。例如`start.sh STM32Lamp`；
...
该项目运作的原理是先运行`start.sh`，运行后会先下载[`/scripts/download.sh`](https://github.com/gaobobo/EduCoder_BigData/blob/main/scripts/download.sh)。

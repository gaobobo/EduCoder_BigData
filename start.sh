...
See more at: github.com/gaobobo/EduCoder_BigData
'
...
error_check() {
    if [ $1 -eq 1 ]; then
        ...
        echo -e "\e[38;5;9;7m 未知的内部错误: \e[0m脚本运行时报告了内部命令错误。如果反复出现可能是脚本问题。" \
                "请至\e[4mgithub.com/gaobobo/EduCoder_BigData\e[0m反馈问题。"
        exit 2
    elif [ $1 != 0  ]; then
...
case $DownloadSite in
    Gitee)
        wget -O /tmp/download.sh https://gitee.com/coconut_floss/EduCoder_BigData/raw/${Branch}/scripts/download.sh
        ;;
    Github)
        wget -O /tmp/download.sh https://raw.githubusercontent.com/gaobobo/EduCoder_BigData/${Branch}/scripts/download.sh
        ;;
esac
